...
export default {
    ...
    setup() {
        ...
        const postSubmit = (url, message) => {
            axios
            //como concatenar para cambiar el tipo de envio
                .post(url, { ...task })
                .then(() => {
                    router.push({
                        name: 'home',
        ...
        function submit() {
            let isParamId = route.params.id ?? false;
            let url = isParamId ? `${urlApi}task/${route.params.id}` : `${urlApi}task`;
            let message = isParamId ? 'Tarea actualizada' : 'Tarea creada';
            
            isParamId ? putSubmit(url, message) : postSubmit(url, message);
        }
